...
function githubHeaders() {
  return { Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", ...(process.env.GITHUB_TOKEN ? { Authorization: `Bearer ${process.env.GITHUB_TOKEN}` } : {}) };
}

function parseRepo(value: unknown) {
  const repo = typeof value === "string" && value ? value : DEFAULT_REPO;
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repo)) throw new Error("Invalid repository format. Use owner/name.");
  return repo;
}

function validateBranch(value: unknown) {
  if (typeof value !== "string" || !value || value === "main" || value === "master") throw new Error("Writes to the default branch are blocked.");
  if (!/^(feature|fix|chore|codex|zvelclaw)\/[A-Za-z0-9._/-]+$/.test(value)) throw new Error("Target branch must use a feature/fix/chore/codex/zvelclaw prefix.");
  return value;
}

function validatePath(value: unknown) {
  if (typeof value !== "string" || !value || value.length > 400 || value.startsWith("/") || value.includes("..") || value.includes("\\") || value.includes("\0")) throw new Error("Unsafe file path.");
  return value;
}

async function github(path: string, init?: RequestInit) {
  if (!process.env.GITHUB_TOKEN) throw new Error("GITHUB_TOKEN is not configured.");
  const response = await fetch(`${GITHUB_API}${path}`, { ...init, headers: { ...githubHeaders(), ...(init?.headers ?? {}) }, cache: "no-store" });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!branchResponse.ok) {
      ...
      await github(`/repos/${repo}/git/refs`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ref: `refs/heads/${branch}`, sha: base.object.sha }) });
    }
    ...
    for (const item of files) {
      ...
      if (action === "create" && existingSha) throw new Error(`File already exists: ${path}`);
      if (action === "update" && !existingSha) throw new Error(`File does not exist: ${path}`);
      if (item?.expectedSha && existingSha && item.expectedSha !== existingSha) throw new Error(`File changed since review: ${path}`);

      const written = await github(`/repos/${repo}/contents/${path}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: `feat(ide): apply ${path}`, content: Buffer.from(content, "utf8").toString("base64"), branch, ...(existingSha ? { sha: existingSha } : {}) }),
      });
