...
type BuildRun = { id: number; status: string; conclusion: string | null; htmlUrl: string; createdAt: string; updatedAt: string };
...
export function AgentIDE() {
  ...
  async function runAgent() {
    ...
    try {
      ...
      const d = await r.json(); if (!r.ok) throw new Error(d.error || "Code engine failed"); setResult(d); setTask(""); setTab("diff");
  ...
  async function applyChanges() {
    ...
    try {
      ...
      const r = await fetch("/api/ide/apply", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ repo: REPO, baseRef: BASE_REF, branch: branchName, files: result.files }) });
  ...
  const statusText = loadingTree ? "Loading repository…" : loadingFile ? "Loading file…" : runningAgent ? "Zvelclaw is thinking…" : applying ? "Applying changes…" : buildState === "running" ? "Build running…" : buildState === "passed" ? "Build passed" : buildState === "failed" ? "Build failed" : "Ready";
  ...
  return <main className="flex h-dvh min-w-0 flex-col overflow-hidden bg-[#0b0d10] text-zinc-100">
    <header className="flex h-12 shrink-0 items-center border-b border-white/10 bg-[#0f1115] px-3 text-sm">
      ...
      <div className="ml-auto flex items-center gap-1"><ToolbarButton icon={<WrenchIcon />} label={applying ? "Applying" : "Apply"} onClick={applyChanges} disabled={!result?.files.length || applying} /><ToolbarButton icon={<SparklesIcon />} label="Review" onClick={() => setPanel("review")} /><ToolbarButton icon={<BotIcon />} label="Agent" onClick={() => setPanel("agent")} /><button onClick={refreshBuild} disabled={!branch} className="ml-1 flex items-center gap-1.5 rounded-md bg-white px-3 py-1.5 font-medium text-black disabled:opacity-40"><PlayIcon className="size-3.5 fill-current" /> Build</button></div>
    </header>
    ...
    <div className="flex min-h-0 flex-1">
      ...
      <section className="flex min-w-0 flex-1 flex-col"><div className="flex h-10 shrink-0 items-center border-b border-white/10 bg-[#101217]"><button onClick={() => setTab("editor")} className={cn("flex h-full items-center gap-2 border-r border-white/10 px-3 text-xs", tab === "editor" ? "bg-[#0b0d10] text-white" : "text-zinc-500")}><FileCode2Icon className="size-3.5" />{activeFile.split("/").pop()}</button><button onClick={() => setTab("diff")} className={cn("flex h-full items-center gap-2 border-r border-white/10 px-3 text-xs", tab === "diff" ? "bg-[#0b0d10] text-white" : "text-zinc-500")}>AI Diff {result?.files.length ? `(${result.files.length})` : ""}</button></div>{error && <div className="border-b border-red-400/20 bg-red-400/5 px-4 py-2 text-xs text-red-300">{error}</div>}<div className="min-h-0 flex-1 overflow-auto font-mono text-[12px] leading-6">{tab === "editor" ? <Editor content={fileContent} loading={loadingFile} /> : <DiffView result={result} />}</div><div className="border-t border-white/10 bg-[#0d0f13]"><div className="flex h-9 items-center gap-4 px-3 text-[11px]"><button onClick={() => setPanel("problems")} className="flex items-center gap-1.5 text-zinc-500"><AlertTriangleIcon className="size-3.5" /> Problems <span className="rounded bg-red-500/15 px-1.5 text-red-300">{error ? 1 : 0}</span></button><span className="flex items-center gap-1.5 text-zinc-500"><TerminalIcon className="size-3.5" /> Terminal</span><span className="ml-auto text-zinc-500">{statusText}</span></div><div className="h-20 overflow-auto border-t border-white/5 px-3 py-2 font-mono text-[11px] text-zinc-500"><div>$ zvelclaw inspect --ref {branch || BASE_REF}</div><div className="text-zinc-400">✓ GitHub repository tree: {tree.length} files</div>{build && <div>CI: {build.status}{build.conclusion ? ` / ${build.conclusion}` : ""} — run #{build.id}</div>}{build?.htmlUrl && <div className="text-zinc-400">{build.htmlUrl}</div>}</div></div></section>

      <aside className="hidden w-96 shrink-0 border-l border-white/10 bg-[#0d0f13] lg:flex lg:flex-col"><div className="flex h-10 items-center border-b border-white/10 px-3"><div className="flex gap-1 text-xs"><PanelTab active={panel === "review"} onClick={() => setPanel("review")}>AI Review</PanelTab><PanelTab active={panel === "agent"} onClick={() => setPanel("agent")}>Agent</PanelTab><PanelTab active={panel === "problems"} onClick={() => setPanel("problems")}>Problems</PanelTab></div></div><div className="min-h-0 flex-1 overflow-auto p-3">{panel === "review" && <ReviewPanel result={result} applying={applying} onApply={applyChanges} buildState={buildState} branch={branch} />}{panel === "agent" && <AgentPanel task={task} setTask={setTask} onRun={runAgent} running={runningAgent} />}{panel === "problems" && <ProblemsPanel error={error} build={build} />}</div></aside>
    </div>
    <footer className="flex h-6 shrink-0 items-center justify-between border-t border-white/10 bg-[#0a0c0f] px-3 text-[10px] text-zinc-500"><div className="flex gap-4"><span>{branch || BASE_REF}</span><span>{result?.files.length || 0} proposed changes</span><span>CI {buildState}</span></div><div className="flex items-center gap-1.5"><CircleDotIcon className="size-3 text-emerald-400" /> Zvelclaw Agent ready</div></footer>
  </main>;
}
...
function ReviewPanel({ result, applying, onApply, buildState, branch }: { result: EngineResult | null; applying: boolean; onApply: () => void; buildState: string; branch: string }) { if (!result) return <Empty icon={<SparklesIcon />} title="AI Review" text="Ask the agent to generate a code change proposal." />; return <div className="space-y-4"><Section title="Summary"><p className="text-xs leading-5 text-zinc-300">{result.summary}</p></Section><Section title="Plan"><ul className="space-y-2 text-xs text-zinc-400">{result.plan.map((x,i)=><li key={i}>• {x}</li>)}</ul></Section><Section title="Changes"><div className="space-y-2">{result.files.map(f=><div key={f.path} className="rounded border border-white/10 p-2 text-xs"><div className="text-zinc-200">{f.action} {f.path}</div><div className="mt-1 text-zinc-500">{f.reason}</div></div>)}</div></Section><button onClick={onApply} disabled={applying} className="flex w-full items-center justify-center gap-2 rounded-md bg-white px-3 py-2 text-xs font-semibold text-black disabled:opacity-50">{applying ? <Loader2Icon className="size-3.5 animate-spin" /> : <CheckCircle2Icon className="size-3.5" />}{applying ? "Applying + building…" : branch ? "Apply again" : "Apply to isolated branch"}</button>{branch && <div className="text-[11px] text-zinc-500">Branch: {branch}<br/>CI: {buildState}</div>}</div>; }
...
function ProblemsPanel({ error, build }: { error: string; build: BuildRun | null }) { return <div className="space-y-3"><div className="text-sm font-medium">Problems</div>{error ? <div className="rounded border border-red-400/20 bg-red-400/5 p-3 text-xs text-red-300">{error}</div> : <div className="text-xs text-zinc-600">No local IDE errors.</div>}{build?.conclusion === "failure" && <div className="rounded border border-red-400/20 p-3 text-xs text-red-300">GitHub Actions build failed. Open the run from the terminal/status area.</div>}</div>; }
